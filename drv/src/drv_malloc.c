...
static __align(64) uint8_t mem2base[MEM2_MAX_SIZE]__attribute__((at(0x30000000))); /* 内部 SRAM1+SRAM2 内存池 */
static __align(64) uint8_t mem3base[MEM3_MAX_SIZE]__attribute__((at(0x38000000))); /* 内部 SRAM4 内存池 */
static __align(64) uint8_t mem4base[MEM4_MAX_SIZE]__attribute__((at(0x20000000))); /* 内部 DTCM 内存池 */
static __align(64) uint8_t mem5base[MEM5_MAX_SIZE]__attribute__((at(0x00000000))); /* 内部 ITCM 内存池 */
/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];
/* 内部 SRAM 内存池 MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0x30000000 + MEM2_MAX_SIZE))); /* 内部 SRAM1+SRAM2 内存池 MAP */
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((at(0x38000000 + MEM3_MAX_SIZE))); /* 内部 SRAM4 内存池 MAP */
static MT_TYPE mem4mapbase[MEM4_ALLOC_TABLE_SIZE] __attribute__((at(0x20000000 + MEM4_MAX_SIZE))); /* 内部 DTCM 内存池 MAP */
static MT_TYPE mem5mapbase[MEM5_ALLOC_TABLE_SIZE] __attribute__((at(0x00000000 + MEM5_MAX_SIZE))); /* 内部 ITCM 内存池 MAP */
#else       /*使用AC6编译器时*/
/* 内存池（64字节对齐） */
...
/* 内存管理参数 */
const uint32_t memtblsize[SRAMBANK] = { MEM1_ALLOC_TABLE_SIZE,MEM2_ALLOC_TABLE_SIZE, MEM3_ALLOC_TABLE_SIZE,MEM4_ALLOC_TABLE_SIZE, MEM5_ALLOC_TABLE_SIZE }; /* 内存表大小 */
...
{
    uint8_t mttsize = sizeof(MT_TYPE);       /*获取memmap数组的类型长度*/
    my_mem_set(malloc_dev.memmap[memx], 0, memtblsize[memx] * mttsize); /*内存状态表数据清零*/
...
static uint8_t my_mem_free(uint8_t memx, uint32_t offset)
{
    ...
    {
        int index = offset / memblksize[memx];      /*偏移所在内存快号码*/
...
void* myrealloc(uint8_t memx, void* ptr, uint32_t size)
{
    ...
    {
        /*拷贝旧内存的内容到新内存*/
        my_mem_copy((void*)((uint32_t)malloc_dev.membase[memx] + offset), ptr, size);
        myfree(memx, ptr);                          /*释放旧内存*/
        return (void*)((uint32_t)malloc_dev.membase[memx] + offset);    /*返回新内存首地址*/
    }
}
