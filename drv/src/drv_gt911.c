...
uint8_t gt9xxx_scan(uint8_t mode)
{
    ...
    if ((t % 10) == 0 || t < 10)/*空闲时,每10次CTP_Scan函数才检测一次，从而节省CPU的使用率*/
    {
        ...
        if ((mode & 0xf) && ((mode & 0xf) <= g_gt_num))
        {
            ...
            res = 1;                /*为什么*/
            if (tp_dev.x[0] > lcddev.width || tp_dev.y[0] > lcddev.width)
            {
...
uint8_t gt9xxx_scan(uint8_t mode)
{
    ...
    {
        gt9xxx_rd_reg(GT9XXX_GSTID_REG, &mode, 1);  /* ¶ÁÈ¡´¥ÃþµãµÄ×´Ì¬ */
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_num))
        {
            temp = 0XFFFF << (mode & 0XF);  /* ½«µãµÄ¸öÊý×ª»»Îª1µÄÎ»Êý,Æ¥Åätp_dev.sta¶¨Òå */
            tempsta = tp_dev.sta;           /* ±£´æµ±Ç°µÄtp_dev.staÖµ */
            tp_dev.sta = (~temp) | TP_PRES_DOWN | TP_CATH_PRES;
            tp_dev.x[g_gt_num - 1] = tp_dev.x[0];  /* ±£´æ´¥µã0µÄÊý¾Ý,±£´æÔÚ×îºóÒ»¸öÉÏ */
            tp_dev.y[g_gt_num - 1] = tp_dev.y[0];
    ...
    if ((mode & 0X8F) == 0X80)  /* ÎÞ´¥Ãþµã°´ÏÂ */
    {
        ...
        else    /* Ö®Ç°¾ÍÃ»ÓÐ±»°´ÏÂ */
        {
            tp_dev.x[0] = 0xffff;
            tp_dev.y[0] = 0xffff;
            tp_dev.sta &= 0XE000;           /* Çå³ýµãÓÐÐ§±ê¼Ç */
        }
    }
