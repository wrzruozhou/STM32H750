...
#ifdef USE_FATTESTER == 1       /*如果使能了文件系统测试*/
    res = mf_init();            /*初始化文件系统测试(申请内存)*/
#endif // USE_FATTESTER == 1
 ...
 * @param       fname : 文件名
 * @retval      文件类型
 *  @arg        0xFF，标识无法识别的文件类型编号
 *  @arg        其他,高4为表示所属的大类，第四位表示所属的小类
 */
uint8_t exfuns_file_type(char* fname)
{
    ...
    for (i = 0; i < 4;i++)tbuf[i] = exfuns_char_upper(tbuf[i]); /*全部变大写*/
    for (i = 0; i < FILE_MAX_TYPE_NUM; i++)     /*大类对比*/
    {
        for (j = 0; j < FILE_MAX_SUBT_NUM; j++)
        {
            if (FILE_TYPE_TBL[i][j] == 0)break;                     /*次组已经没有可对比的成员了*/
 ...
 * @param       psrc:源文件
 * @param       pdst:目标文件
 * @param       totsize:总大小(为0时，表示仅仅为单个文件拷贝)
 ...
 *  @arg        其他，错误代码
 */
uint8_t exfuns_file_copy(uint8_t(*fcpymsg)(uint8_t* panme, uint8_t pct, uint8_t mode), uint8_t* psrc, uint8_t* pdst,
    uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    fbuf = (uint8_t*)mymalloc(SRAMIN, sizeof(FIL));
    ...
    else
    {
        ...
        if (res == 0)res = f_open(fdst, (const TCHAR*)pdst, FA_WRITE | fwmode);     /*只有打开第一个文件才打开第二个文件*/
        if (res == 0)       /*两个文件都打开了*/
        {
            if (totsize == 0)       /*仅仅单个文件复制*/
            {
            ...
            {
                curpct = (lcpdsize * 100) / totsize;            /*得到新的百分比*/
            }
...
#define MAX_PATHNAME_DEPTH (512 + 1)              /*最大目标文件路径 + 文件名深度*/
    ...
    if (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            res = f_opendir(fddir, (const TCHAR*)fdname);   /*打开一个目录*/
            ...
            if (res == 0)
            {
                while (res == 0)                            /*开始复制文件夹里面的东东*/
                {
                    ...
                    if (finfo->fattrib & 0x10)              /*是子目录(文件属性0x20,归档文件;0x10,子目录)，就是说这个目录下面还有一个目录*/
                    {
                        pathlen = strlen((const char*)pathname);    /*得到当前路径长度*/
                        strcat((char*)pathname, (const char*)"/");  /*在后面加上斜杠*/
                        strcat((char*)pathname, (const char*)finfo->fname); /*源路径加上子目录名字*/
                        //printf("\r\nsub folder:%s\r\n",pathname);      /* 打印子目录名 */
                        fdsize += exfuns_get_folder_size(pathname); /*得到子目录大小，递归调用*/
                        pathname[pathlen] = 0;                          /*假如结束符*/
                    }
                    else
                    {
                        fdsize += finfo->fsize;     /*非目录直接加上文件的大小*/
                    }
                }
            }
...
#define MAX_PATHNAME_DEPTH 512+1
    ...
    if (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            if (res == 0)       /*打开目录成功*/
            {
                strcat((char*)dstpathname, (const char*)"/");       /*在文件末尾添加一个斜杠*/
                ...
                res = f_mkdir((const TCHAR*)dstpathname);           /*如果文件夹已经存在，就不创建，如果不存在，就创建新的文件夹*/

                if (res == FR_EXIST)res = 0;

                while (res == 0)                                    /*开始复制文件夹里面的东东*/
                {
                    res = f_readdir(srcdir, finfo);                 /*读取目录的下一个文件*/
                    if (res != FR_OK || finfo->fname[0] == 0)break; /*错误了，到末尾了，退出*/
                    ...
                    srcpathname[srcpathlen] = 0;                        /*假如结束符*/
                    dstpathname[dstpathlen] = 0;                        /*假如结束符*/
                }
            }
